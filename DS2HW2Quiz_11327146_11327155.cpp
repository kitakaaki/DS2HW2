...
class TwoThreeTree {
  private:
    ...
    void searchKMaxHelper(Node* node, int& k, std::vector<int>& result) {
      if (!node) return;

      if (node->count == 2) {
        searchKMaxHelper(node->child[2], k, result);
        if (k <= 0) return;
        result.insert(result.end(), node->keys[1].serial.begin(), node->keys[1].serial.end());
        k -= node->keys[1].serial.size();
      }
      searchKMaxHelper(node->child[1], k, result);
      if (k <= 0) return;
      result.insert(result.end(), node->keys[0].serial.begin(), node->keys[0].serial.end());
      k -= node->keys[0].serial.size();
      searchKMaxHelper(node->child[0], k, result);
    }
  ...
  public:
    ...
    std::vector<int> searchKMax(int k) {
      std::vector<int> result;
      searchKMaxHelper(root, k, result);
      return result;
    }
};
...
class Menu {
  private:
    ...
    void mission3() {
      ...
      std::vector<int> kMaxSerials = twoThreeTree.searchKMax(k);
      IO::displayNodes(makeNodesList(kMaxSerials));
      std::cout << "\n\n";
    }
    ...
    void mission4() {
      ...
      if (searchResultSerials.empty()) {
        std::cout << "\n" << collegeName << " is not found!\n\n";
        return;
      }
      ...
      std::vector<Data> kMaxNodes = searchKMax(nodesList, k);
      IO::displayNodes(kMaxNodes);
      std::cout << "\n\n";
    }
    ...
    std::vector<Data> searchKMax(std::vector<Data>& nodesList, int k) {
      std::vector<Data> result;
      // sort with graduateCount 
